...
ll funcToGetBlocks(int i,vector<ll>& dp,vector<ll>& v,ll& n){
    if(i>n) return 1e10;
    if(i==n) return 0;
    if(dp[i]!=-1) return dp[i];
    ll c1=1+funcToGetBlocks(i+1,dp,v,n); 
    ll c2=funcToGetBlocks(i+1+v[i],dp,v,n);
...
void solve(){
    ...
    vector<ll> dp(n,-1);
    ll ans=funcToGetBlocks(0,dp,v,n);
    cout<<ans<<"\n";
}
