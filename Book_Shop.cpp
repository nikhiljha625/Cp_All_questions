...
int maxmPages(int x,int i,vector<vector<int>>& dp,vector<int>& pages,vector<int>& price){
    if(x<0) return INT_MIN;
    if(x==0||i<0) return 0;
    if(dp[i][x]!=-1) return dp[i][x];
 
    return dp[i][x]=max(maxmPages(x-price[i],i-1,dp,pages,price)+pages[i],maxmPages(x,i-1,dp,pages,price));
...
void solve()
{
    ...
    int ans=maxmPages(x,n-1,dp,pages,price);
    cout<<ans<<"\n";
}
